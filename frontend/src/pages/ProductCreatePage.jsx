...
function normalizeDateTimeLocal(value, fallback = "") {
  ...
  if (value instanceof Date) {
    return Number.isNaN(value.getTime()) ? fallback : toDateTimeLocal(value);
  }

  const valueText = String(value);
  if (valueText.includes("T")) {
    return valueText.slice(0, 16);
  }

  const parsed = new Date(valueText);
  return Number.isNaN(parsed.getTime()) ? fallback : toDateTimeLocal(parsed);
}
...
function createEditForm(product, regionLabel = DEFAULT_REGION_LABEL) {
  ...
  return {
    ...
    pickupStartAt: normalizeDateTimeLocal(product?.pickupStartAt, fallback.pickupStartAt),
    pickupEndAt: normalizeDateTimeLocal(product?.pickupEndAt, fallback.pickupEndAt),
    expiresAt: normalizeDateTimeLocal(product?.expiresAt, fallback.expiresAt),
