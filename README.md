...
| 분류 | 현재 한계                                         | 개선 방향 |
|------|-----------------------------------------------|----------|
| 알림 시스템 | DB 폴링 스케줄러(60초 간격)로 알림 지연 가능성                 | SSE / WebSocket 실시간 푸시 전환 |
| 이미지 처리 | S3에 원본 이미지 그대로 저장 → 고용량 이미지 그대로 서빙            | Lambda 자동 리사이징으로 S3 비용 절감 + 모바일 UX 개선 |
| 검색 | RDB LIKE 쿼리 기반으로 검색 정확도 한계                    | Elasticsearch 도입 → 상품명·가게명 전문 검색 |
| 테스트 | k6 부하테스트만 존재, 단위·통합 테스트 없음                    | 단위·통합 테스트, 베타 테스트 환경 추가 |
| 채팅 | 비밀 댓글 방식으로 판매자·구매자 간 소통 → 비동기적이고 대화 흐름 파악 어려움 | WebSocket 기반 실시간 채팅으로 전환 |

### 인프라 측면

| 분류 | 현재 한계 | 개선 방향 |
|------|----------|----------|
| 배포 전략 | Rolling Update만 적용 | Canary 배포 도입 → 무중단 배포 안정성 향상 |
| 보안 | SG + IAM 기본 설정 | K8s Network Policy + WAF 추가로 계층별 방어 강화 |
| 시크릿 관리 | ExternalSecret + Secrets Manager 연동, 자동 로테이션 없음 | 시크릿 자동 로테이션 정책 추가 |
| DR | 서울 단일 리전(ap-northeast-2) | Multi-Region 대기 구성 또는 Route 53 Failover |
